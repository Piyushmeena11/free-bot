...
async def send_vid(bot: Client, m: Message, cc, filename, thumb, name, prog, channel_id, watermark="Thanos", topic_thread_id: int = None):
    try:
        ...
        thumbnail = thumb
        # Check if thumb is a URL
        if thumb and (thumb.startswith("http://") or thumb.startswith("https://")):
            # Download thumbnail from URL
            try:
                import requests
                temp_url_thumb = f"downloads/thumb_url_{os.path.basename(filename)}.jpg"
                os.makedirs("downloads", exist_ok=True)
                response = requests.get(thumb, timeout=10)
                if response.status_code == 200:
                    with open(temp_url_thumb, 'wb') as f:
                        f.write(response.content)
                    thumbnail = temp_url_thumb
                    # Apply watermark to downloaded thumbnail
                    if os.path.exists(thumbnail) and (watermark and watermark.strip() != "/d"):
                        text_to_draw = watermark.strip()
                        try:
                            import subprocess
                            probe_out = subprocess.check_output(
                                f'ffprobe -v error -select_streams v:0 -show_entries stream=width -of csv=p=0:s=x "{thumbnail}"',
                                shell=True,
                        ...
                        text_cmd = (
                            ...
                            f'-c:v mjpeg -q:v 2 -y "{thumbnail}"'
                        )
                        subprocess.run(text_cmd, shell=True)
                else:
                    thumbnail = None
            except Exception as e:
                print(f"Error downloading thumbnail from URL: {str(e)}")
                thumbnail = None
        elif thumb in ["/d", "no"] or (thumb and not os.path.exists(thumb)):
