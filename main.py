...
# Global variables
watermark = "мєgα яσвσт"  # Always use this watermark
count = 0
userbot = None
...
# 🌐 /proxy command to configure global proxy
proxy_config = {"http": None, "https": None}

@bot.on_message(filters.command("proxy") & filters.private)
async def proxy_cmd(client: Client, message: Message):
    """Add or view current proxy configuration"""
    try:
        # Check if user is admin
        if not db.is_admin(message.from_user.id):
            await message.reply_text("⚠️ You are not authorized to use this command.")
        ...
        if len(args) == 1:
            # No proxy provided — show current config
            current = proxy_config.get("http") or "❌ No proxy set"
            await message.reply_text(
                f"**🌐 Current Proxy:**\n`{current}`\n\n"
                "Use: `/proxy http://user:pass@host:port` or `/proxy clear`"
            )
            return

        new_proxy = args[1].strip()
        if new_proxy.lower() == "clear":
            proxy_config["http"] = None
            proxy_config["https"] = None
            os.environ.pop("HTTP_PROXY", None)
            os.environ.pop("HTTPS_PROXY", None)
            await message.reply_text("✅ Proxy settings cleared.")
            return

        # Validate proxy format
        if not re.match(r"^https?://", new_proxy):
            await message.reply_text("❌ Invalid proxy format.\nExample: `http://user:pass@host:port`")
            return

        # Set proxy globally
        proxy_config["http"] = new_proxy
        proxy_config["https"] = new_proxy
        os.environ["HTTP_PROXY"] = new_proxy
        os.environ["HTTPS_PROXY"] = new_proxy

        await message.reply_text(f"✅ Proxy set successfully:\n`{new_proxy}`")
...
bot.add_handler(MessageHandler(auth.free_cmd, filters.command("free") & filters.private))
...
image_urls = [
    "https://envs.sh/yvq.jpg",
    "https://envs.sh/yvW.jpg",
    "https://envs.sh/yvW.jpg",
    "https://envs.sh/yvS.jpg",
    "https://envs.sh/yv0.jpg",
...
async def start(bot: Client, m: Message):
    ...
    # Start background task on first start
    ...
    [
        InlineKeyboardButton("『ʍɛɢǟ ʀօɮօȶ 』", url="t.me/bandbudh_aur_budbak_megarobot")
            ...
            commands_list = (
                ...
                "**>  /free - ɢᴇᴛ ғʀᴇᴇ 2-ʜᴏᴜʀ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ**\n"
...
def auth_check_filter(_, client, message):
    try:
        ...
        # For private messages
            ...
            # Check if user is authorized (paid subscription)
            if db.is_user_authorized(user_id, bot_username):
                # Check if there's an active free user blocking access
                active = db.get_active_user(bot_username)
                if active:
                    active_user_id = active.get('user_id')
                    # If active user is different and not expired, block access
...
async def txt_handler(bot: Client, m: Message):  
    ...
    # Always use fixed watermark
    global watermark
    watermark = "мєgα яσвσт"
    
    # Always use default credit with hyperlink
    CR = f"{CREDIT}"
    chat_id = editable.chat.id
    timeout_duration = 3 if auto_flags.get(chat_id) else 20
    ...
    thumb = "https://graph.org/file/840f7d2eba833e6be6ce0-c79be50790fdc25499.jpg"
    ...
    # Check if user is free subscription and track upload count
    bot_username = bot.me.username
    active = db.get_active_user(bot_username)
    is_free_user = active and active.get('user_id') == m.from_user.id and not db.is_admin(m.from_user.id)
    
    try:
        for i in range(arg-1, len(links)):
            # Check and stop automatically after 100 links for free users
            if is_free_user:
                # Increment upload count before processing this link
                current_count = db.increment_upload_count(bot_username)
                
                # Check if limit reached (after increment, so it stops after exactly 100)
                if current_count and current_count > 100:
                    await bot.send_message(
                        m.chat.id,
                        f"**🛑 Upload Limit Reached**\n\n"
                        f"You have reached the limit of 100 links for your free subscription.\n"
                        f"Processed: 100/100 links\n\n"
                        f"Processing stopped automatically."
                    )
                    break
            ...
            if "acecwply" in url:
                ...
                url = f"https://covercel.vercel.app/extract_keys?url={url}@bots_updatee&user_id=8111983469"
                mpd, keys = helper.get_mps_and_keys(url)
                url = mpd
                keys_string = " ".join([f"--key {key}" for key in keys])

            elif "classplusapp" in url:
                signed_api = f"https://covercel.vercel.app/extract_keys?url={url}@bots_updatee&user_id=8111983469"
    ...
    f"\n\n<b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>"
                ...
                await bot.send_message(channel_id, f'**━━══ 🏷️ Iɴᴅᴇx ID: {str(count).zfill(3)} ══━━ **\n\n**📑  Tɪᴛʟᴇ** =>> {name1}\n\n📚  𝗕ᴀᴛᴄʜ : {b_name}\n\n** 🔗link  =>> <a href="{link0}">Click Here</a>**\n\n <b>🎓  Uᴘʟᴏᴀᴅ Bʏ : {CR}</b>', disable_web_page_preview=True,
                                       reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("📥 𝙵𝙸𝙻𝙴 𝙻𝙸𝙽𝙺 📁", url=f"{link0}")
            ]
        ])
                                      )
                count += 1
    ...
    channel_id,
    (
        "<b>💥 ℭ𝔬𝔪𝔭𝔩𝔢𝔱𝔢 𝔥𝔬 𝔤𝔶𝔞 𝔪𝔞𝔩𝔦𝔨😎</b>\n\n"
"<blockquote>"
f"<b>📚 𝑩𝒂𝒕𝒄𝒉 𝑵𝒂𝒎𝒆 :</b> {b_name}\n"
f"├ 🖇️ 𝑻𝒐𝒕𝒂𝒍 : <code>{len(links)}</code>\n"
f"├ ✅ 𝑪𝒐𝒎𝒑𝒍𝒆𝒕𝒆𝒅:  : <code>{success_count}</code>\n"
f"├ ❌ 𝑭𝒂𝒊𝒍𝒆𝒅 : <code>{failed_count}</code>\n"
" ╰────────────────\n\n"
"</blockquote>"
f"<i>𝔇𝔬𝔴𝔫𝔩𝔬𝔞𝔡 𝔟𝔶 ➥ {CR}</i>"
    )
)
...
def reset_and_set_commands():
    ...
    {"command": "free", "description": "🆓 use bot free of cost"},
    {"command": "proxy", "description": "upload proxy for indian server"},    
...
async def check_subscription_expiry(client):
    """Background task to check for expired subscriptions and broadcast"""
        ...
        try:
            ...
            if active:
                ...
                if datetime.now() >= expiry:
                    # Clear active user
                    db.clear_active_user(bot_username)
                    
                    # Broadcast to all users
                    await auth.broadcast_bot_available(client, bot_username)
                    
        except Exception as e:
            print(f"Error in subscription expiry check: {str(e)}")
            await asyncio.sleep(60)
